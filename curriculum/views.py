...
class CurriculumView(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if Persona.objects.filter(cedula=request.POST['cedula']):
            self.persona_form.errors = True
            error_general = u'Esta persona ya se encuentra registrada con esa cédula.'
        if Persona.objects.filter(email=request.POST['email']):
            self.persona_form.errors = True
        ...
        if self.persona_form.errors:
            return render(request, 
                           template_name=self.template,
                           dictionary=self.diccionario,
                         )
        else:
             # Si no hay ningún error entonces se crea la persona:
